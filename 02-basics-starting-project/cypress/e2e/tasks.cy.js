...
describe("tasks management", () => {
  ...
  // it("should display an error message when title or summary aren't filled", () => {
  //   cy.visit("http://localhost:5173");
  //   cy.contains("Add Task").click();
  //   cy.get("#summary").type("Some description");
  //   cy.get(".modal").contains("Add Task").click();
  //   cy.get(".modal").get(".error-message").should("exist");
  // });
  ...
  it("should add multiple tasks", () => {
    ...
    cy.get(".task").eq(0).contains("First task").should("exist"); // first()
    cy.get(".task")
      .eq(1)
      .contains(/second task/i)
      .should("exist"); // last()
  });
});
